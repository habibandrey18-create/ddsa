...
from config.link_generation_config import (
    DEBUG_DIR, STORAGE_STATE_DIR, USER_AGENTS, VIEWPORTS,
    DEFAULT_HEADLESS, DEFAULT_TIMEOUT, DEFAULT_MAX_RETRIES, DEFAULT_DEBUG,
    NETWORK_RESPONSE_TIMEOUT, NETWORK_API_PATTERNS, SHARE_BUTTON_SELECTORS, COPY_LINK_BUTTON_SELECTORS, SHARE_MODAL_SELECTORS,
    BROWSER_LAUNCH_ARGS, BROWSER_LOCALE, BROWSER_TIMEZONE, BROWSER_PERMISSIONS,
    HOVER_DELAY_MIN, HOVER_DELAY_MAX, CLICK_DELAY_MIN, CLICK_DELAY_MAX,
    MOUSE_MOVE_DELAY_MIN, MOUSE_MOVE_DELAY_MAX, NETWORK_WAIT_MIN, NETWORK_WAIT_MAX,
    RETRY_DELAY_MIN, RETRY_DELAY_MAX, STORAGE_STATE_HASH_MOD
)
from exceptions.link_generation_exceptions import (
    LinkGenerationError, NetworkError, TimeoutError as LinkTimeoutError,
    HTTPError, CaptchaError, ThrottlingError, ButtonNotFoundError,
    ConfigurationError, RefLinkGenerationError  # Backward compatibility
)
...
class YandexMarketLinkGen:
    ...
    def __init__(
        ...
        # Инициализируем решатель CAPTCHA при создании объекта
    ...
    async def _try_distribution_method(self, url: str) -> Optional[str]:
        ...
        """
        try:
            from utils.partner_link_builder import check_distribution_available, build_partner_link
        ...
        try:
            ...
            if 'captcha' in current_url.lower() or 'yandex.ru/showcaptcha' in current_url.lower() or 'showcaptcha' in current_url.lower():
                ...
                if self._captcha_solver:
                    ...
                    try:
                        from utils.captcha_solver import detect_captcha_on_page, solve_captcha_in_browser
    ...
    async def generate(
        ...
        """
        # Validate inputs
        from utils.input_validators import validate_link_generation_inputs
        ...
        try:
            from utils.xhr_cache import get_xhr_cache
            from utils.xhr_reproducer import reproduce_xhr_directly, extract_short_url_from_response
        ...
        async with async_playwright() as pw:            try:
                ...
                from utils.context_pool import get_context_pool
                ...
                # Проверяем, нужно ли использовать браузер Yandex с существующим профилем                import config
