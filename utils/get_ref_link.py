...
class RefLinkService:
    ...
    async def get_cc_link(self, product_url: str) -> Optional[str]:
        ...
        """
        # ПРОВЕРКА: Если входная ссылка уже содержит /cc/, возвращаем её сразу
        cc_match = re.search(r"/cc/([A-Za-z0-9_-]+)", product_url)
        if cc_match:
            cc_code = cc_match.group(1)
            ref_link = f"https://market.yandex.ru/cc/{cc_code}"
            logger.info(f"✅ Input URL already contains /cc/ code: {cc_code}")
            logger.info(f"✅ Returning existing partner link: {ref_link}")
            return ref_link
